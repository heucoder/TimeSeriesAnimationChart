...
class AnimationChart():
    ...
    def _drawBarChart(self, k):
        # need to update
        dff = self._datasets[self._datasets[self._key].eq(k)].sort_values(by=self._val, ascending=True).tail(self._bar_display_num)
        self._ax.clear()
    ...
    def _drawLineChart(self, k):
        ...
        for name in df[self._name]:
            ...
            self._ax.plot(item[self._key], item[self._val],c = self._colors[name], marker = self._markers[name], lw = 1.5, ms = 3, label = name)
...
def cli():
    ...
    Usage:
        AnimationChart.py (bar | line | pie) <file_path> <val> <key> <name> [--contents=<contents>] [--bar_display_num=<bar_display_num>] [--selected=<selectd>] [--title=<title>]
        AnimationChart.py (-h | --help)
    ...
    key = arguments['<key>']
    name = arguments['<name>']
    contents = arguments['--contents'] if arguments['--contents'] else ""
    ...
    a = AnimationChart(datasets, val, key, name, contents)
